...
手机 overlay 里 y=0 的 `img/标题slg` 是顶栏残留，不算这簇。`≤1126` 日历和首屏主按钮停在手机稿 pageBox（叠放），不要另排一行；`751–1126` 按钮保持 750 稿尺寸，标题 / 日历 / 首屏主按钮整簇按同一 leftover 居中（简中和其它语言同一套 x），列跟窗口铺 KV **和后屏**，750 后屏内容居中，再缩小才等比缩。
