...
| Rule | Meaning |
|---|---|
| Tree switch | Official site and product view (`index.html`) pick the Figma tree from window width. Torchlight official cutoff is `max-width: 1126px`: `0–1126` mobile tree, `≥1127` pc. Desktop narrowing to 1126 and below must become the phone tree. QA uses the same composition breakpoints so a 390 / 800 frame is also the phone tree. Device-picker labels may still map `0–750` / `751–1023` / `≥1024`. Official `is-pc` / `is-mobile` is a UA body class and does not select the tree. No pad tree. |
