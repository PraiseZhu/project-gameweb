...
蓝框显示实际绑定对象。点选按浏览器 `elementsFromPoint` 的叠层顺序取点下第一个内容块，被上层切图、遮罩或弹窗盖住的文字不会被选中。钉视口壳 `__fixed__`、页根 `__page__` 和空的整页「页面内容」只降权、不作为对象；满铺 `bg/` `kv` 只在点下没有其它内容时作为回退。下拉框可以把范围扩大到父级内容块；钉视口壳、页根和空的整页框不进扩大列表，满铺 `kv`/`bg` 仍可作为父级扩大。进入点选时不强制打开这些壳的点击，也不整页扫描节点几何。身份使用完整 data-node 祖先路径；正文修改不改变身份。图钉使用块内归一化位置和当前真实 DOM 几何，跟随缩放、滚动、CSS 动画及 DOM 重建；裁切范围外隐藏。PC/mobile 独立保存，避免不同设计树误配。
...
评论专项测试覆盖点选、父块、拖选、定位、图钉按当前未解决编号、同线程补充、打开评论默认阅读不抢焦点、语言/地区/状态隔离、解决恢复、IndexedDB 失败、标签页同步、QA 壳和产品模式排除、API 存取/认证，钉视口透明壳和空整页框盖住按钮时仍落到文字/按钮，上层切图盖住文字时跟叠层顺序，以及独立模块实例并行写入、缓存空列表后新评论发现、跨秒翻页、250 条旧记录恢复、父记录 replies 损坏仍可见、第 31 条补充不落独立键、分页枚举 `/r/`。旧 v2 评论解决操作保留原正文和作者。
