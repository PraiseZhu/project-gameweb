 ...
 *   node scripts/handoff-promote.mjs --pc <pc.json> --mobile <mobile.json> --confirm "判断已完成" --out <dir>
 */
...
export function runHandoffPromote(argv = process.argv.slice(2)) {
  ...
  if (!args.pc || !args.mobile || !args.confirm) {
    console.error("用法：node scripts/handoff-promote.mjs --pc <pc.json> --mobile <mobile.json> --confirm \"判断已完成\" --out <dir>");
    process.exit(1);
  }
  ...
  const gate = validateHandoffPair(pc.doc, mobile.doc, { allowGreenDraft: true });
  if (!gate.ok) {
