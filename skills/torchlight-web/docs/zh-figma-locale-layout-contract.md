...
- zh-CN: preserve Figma font metrics, geometry, owner hierarchy, and manual line breaks exactly. Static P0 `sc-static-copy-figma` is this rule only. Official-site wrap/weight/resize/interaction rules are language-generic and belong to later axes after static acceptance; do not call them Korean-only.
...
- Source-size tier (measured 2026-08-10, `artifacts/official-tier-ratio-20260810.json`): titles at the same weight split by source font-size into distinct official tiers. `classifySourceSizeTier({ fontWeight, sourceFontSize, ancestorNames, name })` picks the tier: `body` (weight < 600), `card-title` (weight >= 600 and source > 40px), `heading` (weight >= 600 and source <= 40px). Button labels, hero date lines (`sec/1` + `slg`/`标题`), and module-8 claim footnotes (`switch/模块8` + `*每个账号限领取一次`) stay `heading` so they do not take body 0.8. Ratios are `tier × language`, never weight alone:
  - `body`: ja/en/ko 0.8, zh-TW 1.0 (e.g. source 30 -> ja 24).
  - `card-title`: ja 0.833, zh-TW/en/ko 1.0; ja also tightens line-height to ~1.0x font-size.
