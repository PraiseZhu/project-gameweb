...
/**
 * 校验成对真稿清单能不能交出。
 *   node scripts/handoff-validate.mjs --pc <pc.json> --mobile <mobile.json> [--allow-green-draft]
 */
...
export function runHandoffValidate(argv = process.argv.slice(2)) {
  ...
  if (!args.pc || !args.mobile) {
    console.error("用法：node scripts/handoff-validate.mjs --pc <pc.json> --mobile <mobile.json> [--allow-green-draft]");
    process.exit(1);
  }
  const pc = loadInventoryFile(args.pc);
  const mobile = loadInventoryFile(args.mobile);
  const result = validateHandoffPair(pc.doc, mobile.doc, { allowGreenDraft: args.allowGreenDraft });
