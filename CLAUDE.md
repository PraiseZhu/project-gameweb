...
- **Figma 命名稿交接**：命中 `出清单` 后，执行 `standards/figma-naming/SKILL.md`。默认 `npm run inventory` 出 `status: ready`。agent 核一遍后 `handoff:pack` 打 ready 包，再跑做页 `figma:from-handoff`。闸门绿才算交付。对人只交交接包路径，不要把两份 inventory JSON 或核对页链接当交付物。不写回 Figma，不用插件交接。
