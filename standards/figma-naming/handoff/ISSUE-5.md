...
3. 本仓只吃已规范 ready：两端 `npm run inventory` 出 `ready` → `export-handoff-slices`（墨迹框 1 倍 png）→ `handoff:pack`（不要 `--allow-green-draft`）。不要走判断包。
...
- 切图：只导 `img/` `bg/` `kv/`。按节点 `sliceExport`（墨迹框、1 倍、png、文件名=完整 node id）。页上用到的组件集**每个变体**里的切图都要盖住。`assets.ok` 必须为 true。
- 禁止第二套导图：不要再用 `figma-assets.mjs` 的 `use_absolute_bounds=true` 按节点框重导。吃包内 png。
...
跳过 ready 闸门吃 green-draft；已规范稿去走判断包；假清单；unknown 接线；token/大图进 git；改 `standards/figma-naming`；再猜图层名。
