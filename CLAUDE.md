...
- **本仓链路**：已规范设计稿 → 脚本抓 inventory/v2 ready → agent 按 skill 核前缀/结构并打 ready 交接包。做页完成标准：吃 ready 包 → 写出 demo/`index.html` → `preview:first` 必须绿 → 清单对账必须绿（整框 `img/` `bg/` `kv` / 无名 `kv` 的 PNG 非空且宽高等于 `pageBox`）→ 才给人 `?product=1`。Main 静态停下来等人验收。`figma:from-handoff` 只验包、不写 HTML；出页用 `figma:html-from-handoff`。
