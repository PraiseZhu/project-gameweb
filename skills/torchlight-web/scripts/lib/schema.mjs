...
export function validateTruth(truth, { demoDir = process.cwd(), requireProvenance = true } = {}) {
  const problems = [];
  /* A page fixture is commonly referenced by tens of thousands of leaves. Keep
     the per-leaf provenance checks, but hash each source file once per run. */
  ...
  function visit(value, path) {
    ...
    if (isPlainObject(value) && Object.hasOwn(value, 'value')) {
      if (requireProvenance) validateProvenance(value.provenance, path, demoDir, problems, value.value, sourceHashCache);
      return;
    }
...
function validateProvenance(prov, path, demoDir, problems, value, sourceHashCache = new Map()) {
  ...
  if (typeof prov.source === 'string' && prov.source) {
    ...
    if (!existsSync(sourcePath)) problems.push(`${path}.provenance.source 不存在:${prov.source}`);
      ...
      let actualHash = sourceHashCache.get(sourcePath);
      if (!actualHash) {
        actualHash = hashFile(sourcePath);
        sourceHashCache.set(sourcePath, actualHash);
      }
      if (prov.hash && normalizeHash(prov.hash) !== actualHash)
