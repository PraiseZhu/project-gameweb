...
19 项评论专项测试通过，0 失败、0 跳过。覆盖点选、父块、拖选、定位、语言/地区/状态隔离、解决恢复、IndexedDB 失败、标签页同步、QA 壳和产品模式排除、API 存取/认证，以及独立模块实例并行写入、缓存空列表后新评论发现、跨秒翻页和 250 条旧记录恢复。旧 v2 评论解决操作保留原正文和作者。
