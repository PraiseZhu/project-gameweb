...
| 火炬做页 | torchlightweb / 火炬网页还原 | 立即执行 `skills/torchlight-web/SKILL.md`，只跑 `npm run torchlightweb` 状态机，不要先问要不要跑。火炬固定链路不得搜、改、验证 `skills/yise-web-ui` |

用户丢带 `node-id` 的已规范货架链接、且要交给做页时，同样直接跑该 skill。未规范链接不停在本仓硬编，指向 `projects/project-unnamed-inventory`。

## 协作约定

- **本仓链路**：已规范设计稿 → 脚本抓 inventory/v2 ready → agent 按 skill 核前缀/结构并打 ready 交接包。做页完成标准：吃 ready 包 → 写出 demo/`index.html` → `preview:first` 必须绿 → 清单对账必须绿（整框 PNG 非空；满铺 `bg/` `kv` / 无名 `kv` / 时间背景宽高等于 `pageBox`；产品视口首屏无名 `kv` 必须 cover-crop 进 100vh）→ 政策镜像必须绿 → 产品视口门必须绿（390 / 1440 `?product=1`，sec 无缝、满铺子层不重画）→ 像素门必须绿（每屏截已有页对规范稿分区图；超阈值拦，差异图在 `artifacts/stop1-pixel/`）→ 才给人 QA `index.html`（带工具栏）。Main 静态停下来等人验收。火炬触发词只跑 `npm run torchlightweb` 状态机；`figma:from-handoff` 只验包、不写 HTML；`figma:html-from-handoff` 只许编排器调用。火炬固定链路不得搜、改、验证 `skills/yise-web-ui`；共享 `standards/` 源默认只更新火炬转口/测试，不自动改伊瑟副本。伊瑟触发词同样不得去改火炬。
