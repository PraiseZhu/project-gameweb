#!/usr/bin/env node
/**
 * Clean-clone recall: 仓根 CLAUDE.md 触发表 → skills/torchlight-web/SKILL.md。
...
export function recallTorchlightweb(repoRoot = DEFAULT_REPO_ROOT) {
  const claudePath = join(repoRoot, 'CLAUDE.md');
  const skillPath = join(repoRoot, 'skills/torchlight-web/SKILL.md');
  const gitignorePath = join(repoRoot, '.gitignore');
  if (!existsSync(claudePath)) {
    return { ok: false, error: 'missing CLAUDE.md', claudePath };
  }
  const claude = readFileSync(claudePath, 'utf8');
  const hasTrigger = /torchlightweb \/ 火炬网页还原/.test(claude)
    && /立即执行 `skills\/torchlight-web\/SKILL.md`/.test(claude);
  if (!hasTrigger) {
    return { ok: false, error: 'CLAUDE.md trigger table missing torchlightweb → skills/torchlight-web/SKILL.md', claudePath };
  }
  if (!existsSync(skillPath)) {
    return { ok: false, error: 'missing skills/torchlight-web/SKILL.md', skillPath };
  }
...
function main(argv = process.argv.slice(2)) {
  ...
  const result = recallTorchlightweb(root);
