...
| Page scroll lock | Product page height ends at the bg/pc (PC) or bg/mobile (mobile) board bottom (`pageBgBoardBottom()`). Do not cap to the last CTA (that clips the board). Do not `Math.max` content past the board into extra scroll. Content past the board fail-closes with `data-page-scroll-overflow`; do not `Math.min` crop visible content and call it green. Other bg/* slices do not set page height. |
