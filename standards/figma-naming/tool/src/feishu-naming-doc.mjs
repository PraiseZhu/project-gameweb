...
const PREFIX_TIPS = {
  ...
  hot: "@link=  @go=modal/名字  @lang=cn 选填",
  modal: "必须是独立 frame，不要叠在页面稿里",
  dropmenu: "点根切开合菜单，变体精确小写 on/off，不挂 @；PC / 手机都认",
  dyn: "只标容器，里面不用再标前缀",
...
export function buildDesignerDoc() {
  ...
  blocks.push(
    ...
    { type: "bullet", text: "错：在 img/ 上写 @lang；值写成 CN / zh-CN；属性叫 Property 1；只有一个变体；把变体改成 img/标题-cn。" },
    { type: "quote", text: "没画的语言不挡清单。缺图不要回落中文图。@lang 是页上独立按钮的出现条件，不能写在 img/ 上。" },
    { type: "divider" },
    { type: "h2", text: "场景对照" },
    { type: "h3", text: "分区 sec/" },
