#!/usr/bin/env node
// test-demo.mjs — 火炬私有验收套件入口(不再作为公开包无脑可跑的 npm 脚本)。
//
// 这批测试(scripts/__tests__/_*.test.mjs 及若干能力测试)按 process.cwd() 相对定位
// demos/torchlight-preview 与 artifacts/ —— 它们是火炬页面级私有验收,依赖不在发布面的
// 私有 demo/fixtures。公开包使用者直接跑只会得到裸 ENOENT 堆栈,没有任何信息量。
// 因此本入口**必须显式传参** --demo-dir <火炬 demo 目录> 才执行;不传参一律退出并打印说明。
//
// 用法(在火炬工作区内):
//   npm run test:demo -- --demo-dir <火炬工作根>/demos/torchlight-preview
// 或:
//   node scripts/test-demo.mjs --demo-dir <火炬工作根>/demos/torchlight-preview [--dry-run]
//
// --dry-run:只打印将使用的 cwd 与测试文件清单,不真正跑测试(CI/排障用)。
// 执行方式:以「demo 的上一级上一级目录」为 cwd 跑 node --test —— 与测试内
// resolve(process.cwd(), 'demos/torchlight-preview') / 'artifacts/...' 的定位约定一致,
// 不修改任何测试文件。旧路径 demos/yise-ss5-preview 不再作为火炬入口。
...
const USAGE = [
  'test:demo 是火炬私有验收套件,依赖不在公开包内的 demos/torchlight-preview 与 artifacts/ 私有证据,',
  '必须显式指定 demo 路径才会执行(公开包/干净环境不跑它):',
  '',
  '  npm run test:demo -- --demo-dir <火炬工作根>/demos/torchlight-preview',
  '',
  '要求:传入的目录必须是名为 torchlight-preview 的火炬 demo(位于工作根的 demos/ 下,',
  '其父级目录的上一级须包含 demos/torchlight-preview 与 artifacts/ —— 测试按 process.cwd() 相对定位)。',
...
if (basename(demoDir) !== 'torchlight-preview' || basename(resolve(demoDir, '..')) !== 'demos')
  die(`--demo-dir 必须指向名为 torchlight-preview 的火炬 demo(位于 demos/ 下),当前:${demoDir}\n\n${USAGE}`);

/* 测试按 process.cwd() 相对定位 demos/torchlight-preview 与 artifacts/,
   因此以 demo 的上级的上级目录(火炬工作根)为 cwd 跑,与现有私有验收流程一致。 */
const workspaceRoot = resolve(demoDir, '../..');
const demoFromCwd = join(workspaceRoot, 'demos/torchlight-preview');
if (!existsSync(demoFromCwd))
  die(`以 ${workspaceRoot} 为 cwd 找不到 demos/torchlight-preview(测试会按此相对路径定位)——检查传入路径。\n\n${USAGE}`);
