...
| `src/inventory.mjs` | 已规范命名稿 → `inventory/v2`，收页面、同货架 modal、实际引用组件/完整变体与关系 | 无原型或 `@go` 证据的弹窗入口必须保持 `unknown`。无前缀 `lang` 壳仅在变体内恰好一颗时把 `@go` 编成页实例 `modal-trigger`（`lang-shell-variant:@go`，带 `lang`）；0 颗不抬；多颗不抬到页实例，各颗 `@go` 编 `name-param:@go`（带该变体 `lang`）；人读摘要同样打印 `lang=`。独立入口 `@lang` 收成 `langs`，摘要打印 `langs=`；壳内禁止再写 `@lang` |
