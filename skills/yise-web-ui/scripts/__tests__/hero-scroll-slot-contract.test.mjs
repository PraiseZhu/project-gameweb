...
test('generic hero scroll-slot state machine locks, exits progressively, then releases', () => {
  ...
  assert.equal(slot.revealSectionId, 'synthetic-next');
  assert.equal(slot.revealDistance, 1400);
  assert.deepEqual(slot.stateAt(0), { state: 'HERO_LOCKED', progress: 0, scrollTop: 0 });
  assert.equal(slot.stateAt(700).state, 'HERO_EXITING');
  assert.equal(slot.stateAt(1400).state, 'CONTENT_RELEASED');
