...
在 QA 工具栏点击「添加评论」，点选内容或从一个元素拖到另一元素，弹层会显示蓝色范围框。下拉框可以把文字扩大到父级内容块；确认蓝框后输入评论并发布。点选后弹层保持打开，等待输入；点选那一下不会误关窗口，共享同步也不会把正在写的内容清掉。发布后弹层留着，下方可以继续补充，不会新增图钉。点图钉查看，修改页面后刷新，点击「标记已解决」隐藏图钉。已解决条目仍能从列表恢复。详情窗口右上角是关闭窗口的 ×，不会把这条评论标成已解决。图钉数字是当前语言/地区/状态/版式下未解决评论的 1..N，按发布时间排；已解决和其他组合的记录不占号。
...
评论专项测试覆盖点选、父块、拖选、定位、图钉按当前未解决编号、同线程补充、语言/地区/状态隔离、解决恢复、IndexedDB 失败、标签页同步、QA 壳和产品模式排除、API 存取/认证，以及独立模块实例并行写入、缓存空列表后新评论发现、跨秒翻页、250 条旧记录恢复、父记录 replies 损坏仍可见、第 31 条补充不落独立键、分页枚举 `/r/`。旧 v2 评论解决操作保留原正文和作者。
