...
/* per-file caches：extract 一个叶子调一次 makeFixtureLeaf，整页 demo 上万次调用
 * 全打在同一份 fixture 上。不缓存就是 O(叶子数 × fixture 体积)——2026-08-04 整页
 * 11 分区（3000+ 叶子、fixture 2.9MB）实测 extract 3 分钟跑不完，时间全耗在
 * 对同一文件反复 readFileSync+JSON.parse+sha256。缓存键带 mtimeMs：
 * 文件中途被改不会拿旧值（extractor 是纯函数的前提是同一份输入不变）。 */
const _fileHashCache = new Map();
export function sha256File(file) {
  const mtime = statSync(file).mtimeMs;
  const hit = _fileHashCache.get(file);
  if (hit && hit.mtime === mtime) return hit.hash;
  const hash = createHash('sha256').update(readFileSync(file)).digest('hex');
  _fileHashCache.set(file, { mtime, hash });
  return hash;
}
 ...
 */
const _fixtureJsonCache = new Map();
export function validateFixtureValueBinding(value, fixtureAbs, locator) {
  ...
  try {
    ...
    if (cached && cached.mtime === mtime) json = cached.json;
      ...
      _fixtureJsonCache.set(fixtureAbs, { mtime, json });
    }
  } catch (err) {
