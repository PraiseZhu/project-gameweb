...
/* 视口覆盖（figma 场景，任务 16）：默认 1440×960 是老师登录页的尺寸。
   我们的 demo 以 1920 设备打开、壳的 fit 会在容器不足 1920 时整体缩小帧 ——
   截图尺寸就跟着浏览器窗口漂，与 Figma 基线对不上。这里允许 spec 显式指定
   够大的视口让 fit=1（2000 宽 → 可用 1960 ≥ 1920），截图尺寸才确定。 */
const baseVp = spec.baselineViewport || {};
const vpW = Number(baseVp.w ?? baseVp.width ?? 1440);
const vpH = Number(baseVp.h ?? baseVp.height ?? 960);
...
try {
  ...
  page = await browser.newPage({ viewport: { width: vpW, height: vpH }, deviceScaleFactor: spec.baselineDpr ?? 2 });
  ...
  for (const entry of declared) {
    ...
    try {
      ...
      /* ═══ 截图前等就绪（任务 16 实测补的）：渲染层的 <img loading="lazy">
         在视口外不加载 —— boundingBox 的滚动会触发加载，但截图若不等它们完成，
         大图（卡面素材/边框/背景切片）就是白的，diffRatio 直接 52% 假差。
         等两类：①目标元素内全部 img 完成（含 error —— 加载不上也要照常截图报差异）；
         ② document.fonts.ready（webfont 没就绪文字会回退，同样是假差）。
         各带超时兜底：等不到不许卡死门，把未完成数写进 sidecar 供报告点名。 ═══ */
      const readiness = await loc.evaluate(async (el) => {
        el.scrollIntoView({ block: 'nearest' });
        /* The review runtime may defer proof images outside the viewport. Ask
           its generic scheduler to fetch/decode this exact screenshot scope
           before inspecting img.complete; otherwise a deferred img has no src
           yet and would be reported as a false visual regression. */
        if (typeof window.__fxAssets?.prepare === 'function') {
          const pending = window.__fxAssets.prepare(el);
          if (pending) await pending;
        }
        const imgs = [...el.querySelectorAll('img')];
        const waitOne = (i) => (i.complete ? Promise.resolve(i.naturalWidth > 0) : new Promise((r) => {
          i.addEventListener('load', () => r(true), { once: true });
          i.addEventListener('error', () => r(false), { once: true });
        }));
        const settled = await Promise.race([
          Promise.all(imgs.map(waitOne)).then((v) => ({ done: true, okCount: v.filter(Boolean).length })),
          new Promise((r) => setTimeout(() => r({ done: false, okCount: imgs.filter((i) => i.complete && i.naturalWidth > 0).length }), 10000)),
        ]);
        try { await Promise.race([document.fonts.ready, new Promise((r) => setTimeout(r, 8000))]); } catch {}
        return { images: imgs.length, imagesReady: settled.okCount, timedOut: !settled.done };
      });
      ...
      item.sidecar = {
        ...
        /* 截图时就绪状态：图片未加载完成的话差异是假差，这个数字必须先被看见 */
        readiness,
...
const out = {
  ok,
  /* 阈值未拍板前只报不判：ok 照算（看着它），退出码恒 0（不拦人）。
     拍板后删 spec.baselineReportOnly，本字段与放行语义一起恢复正常。 */
  reportOnly,
  skipped: false,
  toolVersion: TOOL_VERSION,
  threshold,
