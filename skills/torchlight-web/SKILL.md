...
description: >-
  火炬之光宣发页 Figma 转网页还原与验收（torchlight-web / torchlightweb）。从 Figma 抽带来源的真值、接线可验收 Demo、跑确定性门并用 Chrome 核验。
  触发词：torchlightweb、torchlight-web、火炬网页还原。本地 Torch demo 只是核验样例，不是 AppStore 应用。
---

<command-name>torchlightweb</command-name>

# torchlight-web - Figma-to-Web UI verification Skill
...
停 1 锁死：整框 PNG 必须有实际像素，不得仅凭尺寸 metadata 判绿；首屏无名 `kv` 使用独立 `coverScale=max(viewportW/designW, slotH/sourceH)`，UI 使用 `pageStageScale`，`cropLeft` 不得二次叠加；viewport `fix/` 根使用 `parentBox` 定位且高度不得乘 hero yScale，子层保持 owner-local。做页不得在清单字段和本包 `DESIGN.md` 之外发明规则：字重/字轴听清单 `fontWeight` + `fontPostScriptName`/`fontStyle`，禁止 `@font-face` `font-named-instance` 钉 Regular 再假粗；`autoResize=HEIGHT` 且只有 `maxWidth` 的 TEXT 必须折行长高，禁止因源中文一行就 `pre` 锁死再缩字号。
...
Do not open the product view, and do not start Interaction / Resize, while `preview:first` is red or the inventory static gate is red. `preview:first` uses HTTP only for the internal check; the URL given to humans is the durable `file://` QA `index.html` path (toolbar included), which must still open after the command exits. Machine gates still measure `?product=1`. Inventory static gate measures `index.html?inventory-static-gate=1` over HTTP for design-viewport coordinates, then `index.html?product=1` after scroll for pin=viewport sticky, spill-slice placement, later-section backgrounds, whole-frame PNG non-empty, full-bleed `bg/` `kv` / unnamed `kv` / time-bg `pageBox` size, and first-screen unnamed `kv` cover-crop into 100vh (PC `center center`, mobile `center 0`). IMAGE descendants under a whole-frame `kv`/`bg`/`img` owner stay inside that PNG; do not export or repaint them. Soft-spill button/logo ink larger than the owner is not a size mismatch. Missing `productScroll`, missing `inventory-static-gate-probe.mjs`, missing `index.html`, or missing Chrome is fail-closed red. Do not hand-edit inventory `status`. Do not treat a local adapter page as Skill acceptance.
...
`torchlightweb` is a stop-layer workflow. Axis order is Main static → Translation
...
1. After Main static (and Translation **only if a copy table is present**):
   `preview:first` must be green, then open the QA `index.html` (toolbar included) and stop. Clicks stay inert until Interaction. That is
...
when they are visually empty. The generic interaction bridge derives
section-target, switch/page, tab/indicator, and hscroll pointer/drag evidence
from those owners; insufficient structure fails closed and is reported rather
than guessed. See `scripts/lib/figma-interaction-contract.mjs` and
`docs/skill-architecture.md`.

Component-set variants are source truth, not a partial-scrape exception. For
each rendered `INSTANCE`, Main Skill must use the **same canonical fixture** to
retain its `componentId`, `componentProperties`, owning `COMPONENT_SET`, raw
property defaults/options, and ordered direct `COMPONENT` variants. A complete
one-to-one set of explicitly `active`/`normal` controls may authorize an
**immediate state replacement** contract; disabled controls are excluded. The
renderer must materialize every alternate tree from that same fixture, and
Chrome must prove the visible replacement; until then this remains
source-ready interaction evidence—not a carousel claim.

For a fixed Figma directory (`fix/*` containing `btn/*` items), Main Skill also
owns scrollspy state. It may wire ordered directory items only when the full
source order has an exact one-to-one section inventory (or when an explicit
source target exists). During scroll it marks the last target above the viewport
midpoint; a clicked target stays selected through smooth scrolling and releases
after 250ms idle or direct wheel/touch/keyboard input. When both Figma
`active`/`normal` variants are present, preserve those exact ornaments while
retaining each instance label. Missing targets, mismatched counts, or missing
variants remain unresolved rather than receiving guessed links or CSS states.

### Extraction completeness is a gate

Do not declare a Main Skill run complete from a non-empty or partial Figma
scrape. The run must inventory all page-frame siblings and nested component
owners, preserve `parentId`/`ownerPath`/`orderKey` plus source provenance, and
provide an explicit reason for every skipped subtree. Component/interaction
coverage and real Chrome render/interaction evidence are required. Incomplete
fetch scope, unexplained page-scope filtering, or flattened owner trees stay
...
After it passes, open QA `index.html` immediately for human review. Do not send humans `?product=1`. A newer
...
- section/page/fixed stage 的真实来源位置；
- 节点 `box` 的 x/y/w/h；
- owner-local `parentId` / `ownerPath`；
- Auto Layout owner 的 padding 与 HUG/FIXED 消费；
- 节点是否缺失、隐藏、零尺寸或被错误的 owner/flow 推移。

当前 demo 强制覆盖 3840×2160 PC 状态的 02 奖励区和固定目录。02 至少要核对卡片组、卡片容器、标题、奖励列表、三段底部说明文字；目录至少要核对固定 owner、背景和 source-ordered 导航按钮。失败报告必须保留 expected/actual/delta/ownerPath，并生成局部截图；不得用“整体截图看起来差不多”替代。

这道门必须注册为 `spec.customGates`，进入门 X 的可信执行和 hash 链；最终验收不得只跑局部过滤命令。静态数据链、Node renderer smoke、源几何 Chromium、交互/适配、像素基准分别回答不同问题，任何一层失败都必须如实报告。

**验收顺序固定为**：Figma 重建 → truth/provenance/schema → 源几何静态断言 → Node renderer smoke → 真实浏览器源几何/交互 → pixel baseline → 全量 canonical verify/pr-block。源几何门失败时，即使 A-F/X 或 hscroll 通过，也不得声明页面可验收。
...
**外部使用者**：canonical public identity 是 `torchlight-web`。本轮只做本地发布边界准备，未创建或发布 GitHub 仓库；台账条目如需回流，只动 `evolution/ledger.json`（经 evolution-note.mjs add 生成），不改判定脚本。
...
验收自动化（verify.mjs、门 B/C/D/F、清单对账、像素门）仍测 `?product=1` / `__qa`，
那是机器路径，不是给人看的入口。
