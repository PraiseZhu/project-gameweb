...
偏移之后（正负偏移都要续，不能只在 extra>0 时画），页面背景跟着后屏走，
滚动锁跟这份视觉终点，不会露出空带。KV/page chrome 与 fixed overlay 仍按
...
不会被 `y×k` 抬到上半屏，也不会浮在中间。`slg` / 日历 icon / 播放按钮 / 首屏主按钮 / 赛季福利是同一竖向簇：共用一次**页坐标**底边位移，稿上相对位置不变。套在 `标题` 组里的 `img/标题slg` 和播放按钮跟组一起钉，不要把页位移写成局部 top。`≤1126` 钉奖卡底边到 100vh：槽比稿矮整簇上提（播放可以出槽顶，不要为了留播放把奖卡吞掉），槽比稿高往下垫（不要让奖卡上飘）。不要各钉各的。PC 冻档 `slg` 就是 `k=0.5` 整体缩放再居中裁；`>1920` 也只走页 `k`，不要再按窗口宽二次拉伸。
手机 overlay 里 y=0 的 `img/标题slg` 是顶栏残留，不算这簇。`≤1126` 日历和首屏主按钮停在手机稿 pageBox（叠放），不要另排一行；`751–1126` 按钮保持 750 稿尺寸，标题 / 日历 / 首屏主按钮整簇按同一 leftover 居中（简中和其它语言同一套 x），列跟窗口铺 KV；后屏组件和长 `bg/mobile` 按 `windowW/750` 等比，不要只横向铺开。page 裁切跟窗口，后屏 section 仍是 750 稿宽再吃 page zoom；不要把 page 锁 750 再 clip。后屏 Y 在后屏尺子上贴 used 100vh（`designTop = viewportH/laterScale`），不要把 k 边再乘 page zoom；KV cover 用首屏 k 量窗口，不要用后屏尺子。
