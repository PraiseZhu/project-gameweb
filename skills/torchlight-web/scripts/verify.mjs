#!/usr/bin/env node
// verify.mjs — torchlight-web 门 A/B/C/D/F/X 动态验收执行器。
//
...
try {
          ...
          const failures = await p.evaluate((selectors) => {
            ...
            for (const sel of selectors) {
              ...
              for (const el of all) {
                ...
                /* 滚动容器（overflow:auto/scroll 且真有溢出）的 scroll>client 是**合法可滚动**，
                   不是截断——整个 .frame 就是页面的滚动视口。只把「容器被裁且无滚动出口」
                   的隐藏/visible 溢出当真截断。2026-08-08 前 no-clip 把 .frame 自身的滚动
                   溢出误报成「纵向截断」，每个 case 都挂。 */
                const oy = s.overflowY, ox = s.overflowX;
                const canScrollY = (oy === 'auto' || oy === 'scroll');
                const canScrollX = (ox === 'auto' || ox === 'scroll');
                const rawClipX = el.scrollWidth > el.clientWidth + 1;
                const rawClipY = el.scrollHeight > el.clientHeight + 1;
                const clippedX = rawClipX && !canScrollX;
                const clippedY = rawClipY && !canScrollY;
  ...
  function runDemoScript(scriptAbs, extraArgs = [], { timeout } = {}) {
    ...
    const res = spawnSync(process.execPath, [copy.exec, ...extraArgs], {
      cwd: demoDir,
      encoding: 'utf8',
      maxBuffer: 128 * 1024 * 1024,   // truth 全量（含 prototype/ownerPath 结构证据）已达 ~48MB，32MB 会 ENOBUFS 误判成 extract 执行失败
