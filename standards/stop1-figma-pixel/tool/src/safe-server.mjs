...
export function createSafeStaticServer(rootDir) {
  ...
  return {
    ...
    async listen(host = '127.0.0.1') {
      ...
      /* 返回**无尾斜杠**的 base（2026-08-05 修双斜杠 404）：
         之前带尾斜杠 http://host:port/，调用方再拼 base + '/index.html' 得 //index.html ——
         浏览器 docBase 带双斜杠后，页内相对路径 assets/x.png 解析成 //assets/x.png，
         toSafeFile resolve 后与 root 前缀不匹配 → 全部 404（lazy 图加载失败、轮播箭头缺失）。
         无尾斜杠时：page.goto(base) 的 req.url='/' 仍映射 index.html；
         base + '/index.html' = /index.html 单斜杠；new URL('assets/x', base) 也正确。 */
