...
test('renderer exposes the generic state contract and does not use a visual cover', () => {
  ...
  assert.match(render, /Nested stages inherit page zoom/);
  assert.match(render, /pageScope/);
  assert.match(render, /isHeroStage \? heroCounterZoom : 1/);
  ...
  assert.match(render, /heroLayoutOffsetDesign \+ heroViewportH \* \(1 \/ Number\(laterUniformScale\) - 1 \/ Number\(k\)\)/);
  assert.match(render, /Join later to the used hero bottom on the later/);
  assert.match(render, /pageStageMode \|\| isHeroStage/);
  assert.match(render, /const laterLayerW = windowStageWidth/);
  assert.match(render, /keeps zoomed later art at 750 CSS/);
  assert.match(render, /_laterUniformScale/);
  assert.match(render, /data-later-uniform-scale/);
  assert.match(render, /data-hero-ui-counter-zoom/);
  assert.match(render, /data-later-cover-axis', 'uniform'/);
  assert.match(render, /Nested stages inherit page zoom/);
  assert.match(render, /laterJoinOffsetDesign/);
  assert.match(render, /data-later-join-offset-design/);
