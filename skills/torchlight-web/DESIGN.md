...
本文件是火炬做页的**政策入口**。清单仍是数据入口：它回答这一稿画了什么。本文件回答窗口怎么切、尺子怎么量、外文怎么缩。数字写在这里，不焊进 inventory JSON，也不替代 `figma:from-handoff`。政策数字以文首 YAML 为准；第 5、6 章只解释，不再另写一套可漂的数。产品切树是 `composition`（0–1126 / ≥1127），QA 样品桶是 `qaBuckets`，两表不得并成一张。
...
- 官方站 CSS 是行为参考，不是第二份政策。季节海报样式不进本文件。官方 `is-pc` / `is-mobile` 是 UA body class，不选树。
...
官方站 `max-width: 1126px` 是切树阈值：桌面缩到 1126 及以下必须换手机树。**不写 pad 树，不发明 pad 树。** 设备选择器标签仍可映射 `0–750` / `751–1023` / `≥1024`，那只是 QA 样品桶，不是产品页切树。
...
- 不要另写一份「官方站 CSS 政策」。官方站是参考，季节补丁不进本文件。1126 只切树，不把那条 media-query 的 display/size 规则变成第二套布局。
...
4. 拉伸主张要带视口 `w×h`、实际用的 composition key（火炬：≤1126 手机，≥1127 PC）、light path 还是全量重建、view-fit scale。不得用 UA `is-pc` / `is-mobile` 当切树证据。
