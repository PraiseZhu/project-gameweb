...
| Light drag | Continuous edge-drag / slider may skip content rebuild only on the same composition base. Freeze-band `k` stays 0.5 during the drag (do not scale UI by `nextW/lastW`). The 100vh slot still follows the current drag height (`hero height = viewportH / current zoom`); later sections stay at that edge. Language, device, window resize, and W/H box stay on the full path. An open named modal stays mounted: light drag re-pins it to the current frame; pointerup full rebuild snapshots `data-modal-name` before the wipe and restores via `__fxOpenNamedModal`. Tree switch at 1126 restores the matching mobile/PC sheet by topic (`pc_cn订阅赛季日程` → `mobile订阅赛季日程`, `pc_播放弹窗` → `mobile视频弹窗`); it does not close. Official popup is `position:fixed` and survives resize. Above 1920, named lock-1920 layers follow page `k` / `followScale`; do not re-apply freeze `counter = 0.5 / currentK` during the drag. |
| Temporary lock-1920 names | Not a lasting naming system, not `@fit=`. Exact names for later seasons until naming research lands: `fix/` prefix, COMPONENT_SET `btn/主要按钮`, COMPONENT_SET `首屏主按钮`. Identity matches the set name / `fix` prefix, never a page instance `btn/按钮`. Above 1920 these layers follow page `k` with the rest of the PC tree so relative size vs KV stays the freeze-band ratio; do not freeze CSS size at `k=0.5` or counter-scale each button (that shrinks Shop vs KV, un-centers the 充值 label, and opens top-bar gaps). Landscape `fix/` is viewport chrome: official top bar is `position:fixed; width:100%; justify-content:flex-end` on both `1127–1920` and `>1920`, so the Figma right edge sits on the current window, not the frozen 1920 column. Keep the authored CN right-edge inset every season. CN PC top-bar buttons flush the painted `img/按钮背景` to `img/奖励底框` right plus any overflowing child of that plate, not a season node-id; mobile CN `fix/右上方导航` pins to the window right. Resize consumes the Interaction region shell (CN inset, mobile 进入官网 on the language-icon slot) and does not invent those names. Pin the design page right, not the visual leaf, or a short mobile `fix/` group eats that inset. Overlay shop / globe / dropmenu are siblings of the wide `fix/` group, not nested children. Overlay host is already viewport-wide, so left overlay chrome keeps source x; page-tree left chrome (`img/LOGO`, `btn/年龄`) cancels freeze `pageStageCropLeft` so the source inset is from the window left. Authored `fix/下滑箭头` follows the window midpoint even when its Figma x is the 3840/1920 column center. Bare `btn/按钮` is the first-screen download CTA, not top-bar chrome. `1127–1920` already uses `k=0.5`. Mobile stays on the phone tree. KV cover still stretches with the window. Freeze-band SLG stays `k=0.5` overall scale; above 1920 SLG stays page `k` (no extra viewport-width stretch). First-screen `slg` / calendar / play / primary CTA / mobile `btn/按钮` / flattened 赛季福利 group keep one vertical cluster in **page coords**; nested title SLG and play ride the `标题` owner. Do not remap those members independently. `≤1126` calendar + CTA stay on Figma pageBox (overlapping stack at 62 / 133); do not invent a row. `≤1126` first-screen UI shares `k=viewportW/750` with later so leftover extra is 0 above 750 (do not freeze 750 then leftover-center). Gradient text must keep the leftover already written on `left`; named modal paint never takes page leftover. Every language uses the same pageBox x/y as zh-CN, including later sections at `≤1126` and the freeze band. Then only shrinks. A language-shell remount of `首屏主按钮` must not add leftover a second time onto nested button art — that clips 立即下载 on the right. `≤1126` later UI and the long `bg/mobile` board share `windowW/750` uniform scale plus the same `layoutOffsetDesign` so relative stretch matches official; the scroll lock follows that visual end. Freeze-band KV still covers the real viewport; QA wrap is `vp.w`, not the 1920 column. |
