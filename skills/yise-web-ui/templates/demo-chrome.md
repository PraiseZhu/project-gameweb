...
- demo 必须提供纯净入口（如 `index.html?product=1` 或等价参数/独立入口）——切换器、状态补齐 tab、
  拉伸手柄、工具区文字全部隐藏，只剩产品帧。这条路径只给脚本探测和无壳截图，
  **不是给人看的默认入口**。给人看的地址停在 `index.html`（QA 壳，含 PC/手机/语言切换）。
  生成模板未内置该路径时，实现阶段（P2）必须自行提供；验收/交付截图一律走产品视口，
  文件名按约定区分（`*-product.png` vs `*-qa-shell.png`），验收时须证明工具区不在视口内。
...
`由 yise-web-ui 生成 · truth 带 provenance · A-F 门验收通过 <日期>`
