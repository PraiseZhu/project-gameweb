...
| 火炬做页 | torchlightweb / 火炬网页还原 | 立即执行 `skills/torchlight-web/SKILL.md`，只跑 `npm run torchlightweb` 状态机，不要先问要不要跑 |

用户丢带 `node-id` 的已规范货架链接、且要交给做页时，同样直接跑该 skill。未规范链接不停在本仓硬编，指向 `projects/project-unnamed-inventory`。

## 协作约定

- **本仓链路**：已规范设计稿 → 脚本抓 inventory/v2 ready → agent 按 skill 核前缀/结构并打 ready 交接包。做页完成标准：吃 ready 包 → 写出 demo/`index.html` → `preview:first` 必须绿 → 清单对账必须绿（整框 PNG 非空；满铺 `bg/` `kv` / 无名 `kv` / 时间背景宽高等于 `pageBox`）→ 政策镜像必须绿 → 产品视口门必须绿（390 / 1440 `?product=1`，sec 无缝、满铺子层不重画）→ 才给人 `?product=1`。Main 静态停下来等人验收。火炬触发词只跑 `npm run torchlightweb` 状态机；`figma:from-handoff` 只验包、不写 HTML；`figma:html-from-handoff` 只许编排器调用。
