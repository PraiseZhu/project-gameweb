...
const PREFIX_TIPS = {
  ...
  img: "普通文字不要加这个。按语言换图：做成组件集，变体属性名 lang，至少两个不同的精确小写 cn/tw/en/jp/kr。单变体、CN、xx 不跟语言。不要写 @lang",
  bg: "—",
...
export function buildDesignerDoc() {
  ...
  const blocks = [
    ...
    { type: "bullet", text: "点击 / 滑动 / 切换 → btn/　hot/　scroll/　switch/　tab/　ind/　modal/" },
  ...
  blocks.push(
    ...
    { type: "bullet", text: "switch/ 是容器，tab/ 是页签，ind/ 是圆点。" },
    { type: "bullet", text: "一组 ind/ 可以全叫同一个名字，序号按同级顺序认。" },
    { type: "bullet", text: "同一分区里要能唯一对上一个 switch/：套在里面，或旁边只有一个，都可以。" },
    { type: "bullet", text: "分区里没有轮播，或有多个却对不上，会报错。组件库定义不参与配对，页面上的实例要。" },
    { type: "h3", text: "滑动 scroll/" },
    { type: "bullet", text: "第一个子层是内容轨道。" },
    { type: "bullet", text: "轨道里的图仍要 img/、bg/、kv/，不因为外层是 scroll/ 就免。" },
    { type: "h3", text: "图文混排 mix/" },
    { type: "bullet", text: "只标容器。里面的图不用再标前缀，清单会自动按 img/ 切。" },
    { type: "bullet", text: "里面的字仍当可改文案，不要整块切 mix/。" },
    { type: "bullet", text: "scroll/ 写在 mix/ 里仍是滑动裁切，不因为外层是 mix/ 就整块切。" },
    { type: "h3", text: "弹窗 modal/" },
    { type: "bullet", text: "做成独立 frame。" },
    { type: "bullet", text: "不要直接叠在页面稿里。" },
    { type: "h3", text: "KV 视差" },
    { type: "bullet", text: "同一父层要有多层才叫视差。" },
    { type: "bullet", text: "只有一层，用 img/。" },
    { type: "h3", text: "有图填充但没前缀" },
    { type: "bullet", text: "带图像填充的叶子，通常要标 img/、bg/、kv/。" },
    { type: "bullet", text: "例外：最近前缀已是 ind/ 时，里面的小圆点不用再标。若 ind/ 里又套了 btn/，仍按 btn/ 来。" },
    { type: "h3", text: "同名" },
    { type: "bullet", text: "同一页里，两个不同图层不要用完全相同的名字（含前缀）。" },
    { type: "bullet", text: "不要靠 img/头像-1、img/头像-2 区分，名字要能说明它是什么。ref/ 子树和组件变体的强制名除外。" },
    { type: "divider" },
    { type: "h2", text: "报警对照" },
  ...
  for (const chunk of chunkTable(["报警码", "你要做什么"], mustChangeRuleRows().map((r) => [r.code, r.fix]))) {
