...
export async function loadPngApi(startDir) {
  ...
  try {
    ...
    const mod = await import(pathToFileURL(pixelmatchPath).href);
    pixelmatch = mod.default ?? mod;
  } catch {}
  let odiff = null;
  try {
    const odiffPath = resolveModule('odiff-bin', startDir);
    const mod = await import(pathToFileURL(odiffPath).href);
...
export function comparePngs({ PNG, pixelmatch, baseline, actual, cssSize, masks = [], threshold = 0.005, maxMaskRatio = 0.40, minUnmaskedRatio = 0.5 }) {
...
async function comparePngsOdiff({ PNG, odiff, baselineRaw, actualRaw, cssSize, masks = [], threshold = 0.005, maxMaskRatio = 0.40, minUnmaskedRatio = 0.5 }) {
