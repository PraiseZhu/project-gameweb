...
在 QA 工具栏点击「添加评论」，点选内容或从一个元素拖到另一元素，弹层会显示蓝色范围框。下拉框可以把文字扩大到父级内容块；确认蓝框后输入评论并发布。点选后弹层保持打开，等待输入；点选那一下不会误关窗口，共享同步也不会把正在写的内容清掉。发布后弹层留着，默认只显示线程和底部「回复」，不会把光标送进补充框。点图钉或全部评论里的条目同样先阅读；只有点「回复」才在线程底部展开输入。补充仍追加到这条评论，不覆盖原文，也不另开图钉。点图钉查看，修改页面后刷新。「标记已解决」是详情窗口右上角的勾选，和关闭窗口的 × 同一组工具栏，不和「回复」并排。点解决后详情和图钉立刻消失，这条评论只留在全部评论的已解决筛选里，可从那里恢复；从已解决列表再打开时，刷新和跨标签同步不会把窗口关掉。右上角 × 只关窗口，不会把这条评论标成已解决。图钉数字是当前语言/地区/状态/版式下未解决评论的 1..N，按发布时间排；已解决和其他组合的记录不占号。具名弹窗打开时，屏幕上只显示弹窗这一层的图钉和蓝框，底下页面的先隐藏。从列表点到底下那层的评论时，先关上弹窗再定位，避免详情还标着被挡住的内容。关上弹窗后再露出页面图钉。

「全部评论」点开后保持展开，直到点关闭列表、再点「全部评论」，或在没有详情、也没有点选时按 Esc。默认汇总当前页面所有语言、地区、状态、版式的未解决条目。点击条目会切回对应组合，并沿评论 `data-node` 祖先打开具名弹窗、切到所在 `switch/` 页、组件集变体屏或轮播页，再滚动到内容；列表不因此收起，也不自动进入补充输入。写新评论或打开详情时，列表仍可同时开着；未发布的草稿不会因为打开列表或再点「添加评论」被清掉。Esc 先关详情，再退出点选，最后才关列表。也可筛选当前组合和已解决条目。平铺状态下须先退出平铺才能创建；从列表打开条目会自动退出平铺。点空白满铺 `kv`/`bg` 时落到该层，而不是提示没有可绑定内容；空的整页「页面内容」和钉视口壳仍不作为默认对象。
...
蓝框显示实际绑定对象。点选先按浏览器 `elementsFromPoint` 的叠层顺序取点下第一个可点内容块，被上层切图、遮罩或弹窗盖住的文字不会被选中。标题、文案若被渲染成 `pointer-events: none`，再按几何取点下最小的可见非壳内容，隐藏、透明或被裁切的节点不算；没有可见内容才落到满铺 `kv`/`bg`。钉视口壳 `__fixed__`、页根 `__page__` 和空的整页「页面内容」只降权、不作为对象；满铺 `bg/` `kv` 只在点下没有其它内容时作为回退。下拉框可以把范围扩大到父级内容块；钉视口壳、页根和空的整页框不进扩大列表，满铺 `kv`/`bg` 仍可作为父级扩大。进入点选时不强制打开这些壳的点击。身份使用完整 data-node 祖先路径；正文修改不改变身份。图钉使用块内归一化位置和当前真实 DOM 几何，跟随缩放、滚动、CSS 动画及 DOM 重建；裁切范围外隐藏。具名弹窗打开时，不在该弹窗里的图钉和蓝框也先隐藏；从列表点到底下那层时先关窗再定位。PC/mobile 独立保存，避免不同设计树误配。

同一路径重复、原节点删除、类型改变时不放置图钉；评论保留在列表并说明原因。不能复用一个既有 ID 表示无关内容。此版绑定内容块，不是字符级选区；文字重排时锚点跟随该文字元素框。任意手绘形状及图片附件不在本版范围。点列表定位时，先按祖先打开关着的具名弹窗、切到 `data-switch-page` / `data-switch-variant-index` / `data-motion-carousel-page`；页面内容再 `scrollIntoView`，已打开的具名弹窗不再滚隐藏层（关层高度为 0，滚它会回到首页）。不另存一份弹窗名或轮播下标，旧评论也能还原。仍关着的弹窗会显示「内容当前不可见」，因为关层是 `hidden` + `aria-hidden`，不是滚出视口。

主功能接入 templates/figma-chrome.js；现有机械内联/生成链会携带功能。已部署的旧 HTML 不会自行升级。?product=1 和 ?inventory-static-gate=1 不注入评论界面。列表还原走 `data-modal-name` / `data-switch` / `data-switch-variant-index` / `data-motion-carousel-page`，不写本季 Figma id。`commentPageId`（如 `ss14-season-phase2-qa`）只隔离这一站的库，不是 skill 规则。

## 部署

评论壳在打包进页的 `figma-chrome.js` 里，不单独热更新。同事要看到新点选 / 列表还原，必须把带新壳的 HTML 部署到那一站；只改 skill 仓、只发评论 Worker，线上页还是旧的。

- 共享评论：页上声明稳定 `commentPageId` + 同站 `commentApi`（通常 `/api/qa-comments`）。同一 pathname、不同稿必须换 pageId，否则库会串。pageId 是这一站的配置，不是 Figma id。
- 没配 `commentApi`：只有本机 IndexedDB。本地 zip / `file://` / 纯静态 HTTP 没有 Worker，黄字 `Failed to fetch` 是预期，不是页坏了。
- 评论 API Worker（`deploy/qa-comments-realtime`）管存取和 SSO，不管壳。壳随页走，API 随 Worker 走，两套部署，不要混成一次。
- 保险副本：已有人在用的 live 站，skill 改完不等于授权覆盖。要发再说一声。Pack 的 `out.hashes` 对上部署文件，才能说线上是这一版。
- `?product=1` / `?inventory-static-gate=1` 不带评论壳。
...
评论专项测试覆盖点选、父块、拖选、定位、图钉按当前未解决编号、同线程补充、打开评论默认阅读不抢焦点、语言/地区/状态隔离、解决恢复、IndexedDB 失败、标签页同步、QA 壳和产品模式排除、API 存取/认证，钉视口透明壳和空整页框盖住按钮时仍落到文字/按钮，上层切图盖住文字时跟叠层顺序，标题文案 `pointer-events: none` 时仍落到可见文字而不是满铺 kv，隐藏语言文案不算，列表点击会重开具名弹窗（含关层高度为 0 的真实 overlay）并切到对应轮播页，弹窗打开时只显示弹窗层图钉，以及独立模块实例并行写入、缓存空列表后新评论发现、跨秒翻页、250 条旧记录恢复、父记录 replies 损坏仍可见、第 31 条补充不落独立键、分页枚举 `/r/`。旧 v2 评论解决操作保留原正文和作者。
